...
Nginx sử dụng mô hình xử lý kết nối dựa trên events, do đó các lệnh được xác định trong ngữ cảnh được xác định cách các quy trình làm việc nên xử lý cách kết nối. Context được tìm thấy ở đây được sử dụng để chọn kỹ thuật xử lý kết nối sẽ sử dụng hoặc để sửa đổi cách thức thực hiện các phương thức này.
...
Mặc dù bối cảnh thấp hơn sẽ cụ thể hơn về cách xử lý các yêu cầu, các chỉ thị ở cấp độ này kiểm soát các mặc định cho mọi máy chủ ảo được xác định bên trong. Một số lượng lớn các lệnh có thể được cấu hình trong bối cảnh này bên dưới, tùy thuộc vào cách bạn muốn kế thừa hoạt động.

Một số các chỉ thị rằng bạn có khả năng kiểm soát cuộc gặp gỡ các vị trí mặc định cho truy cập và lỗi bản ghi(access_log và error_log), cấu hình không đồng bộ bật tắt cho các hoạt động tập tin( aoi, sendfile và directio ), cấu hình trạng thái máy chủ khi xảy ra lỗi(error_page). Chỉ thị cấu hình nén (gzip và gzip_disable), Tinh chỉnh TCP giữ các thiết lập còn hoạt động(keeplive_disable, keepalive_requests và keepalive_timeout), và các quy tắc mà Nginx sẽ đi theo để cố gắng gói tối ưu hóa và các cuộc gọi hệ thống(sendfile, tcp_nodelay và tcp_nopush). Các chị thị bổ sung định cấu hình tệp tệp chỉ mục và gốc tài liệu ở cấp ứng dụng (root và index) và thiết lập các bằng hash khác nhau( *_hash_bucket_size và *_hash_max_size cho Server_names, types và variables ).
...
Lý do cho phép nhiều khai bào về bối cảnh máy chủ là mỗi trường hợp xác định một máy chủ ảo cụ thể để xử lý các yêu cầu của máy khách. Bạn có thể có nhiều khối máy chủ mà cần. mỗi khối có thể xử lý một tập hợp con cụ thể của các kết nối.

Do khả năng của nhiều máy chủ, Loại bối cảnh này cũng là lần đầu tiên Nginx phải sử dụng thuật toán lựa chọn để đưa ra quyết định. Mỗi yêu cầu máy khách sẽ được sử lý theo  cấu hình được xác định trong một ngữ cảnh máy chủ, do đó Nginx phải quyết định bối cảnh máy chủ nào phù hợp nhất dựa trên chi tiết của yêu cầu. Các chỉ thị quyết định nếu một khối máy chủ sẽ được sử dụng yêu cầu :
...
* server_name: Lệnh này thành phàn khác được sử dụng để chọn khói máy chủ để xử lý. Nếu có nhiều thành phần khối máy chủ có chỉ thị lắng nghe có cùng độ đặc hiệu có thể xử lý yêu cầu, Nginx sẽ phân tích tiêu đề của máy chủ lưu trữ tren máy chủ của yêu cầu và khớp với chỉ thị này.
...
mặc dù Nginx thường được sử dụng làm web server hoặc reverse proxy server, nó cũng có thể hoạt động như một mail proxy server hiệu suất cao. context được sử dụng cho các chỉ thị loại này được gọi một cách thích hợp là thư mail. mail context được xác định trong bối cảnh chính tren toàn global hoặc main.
